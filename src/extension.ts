...
export function activate(context: vscode.ExtensionContext) {
  ...
  const markCurrentPos = (unmark_if_present: boolean, posOverride?: vscode.Position) => {
    ...
    vscode.commands.executeCommand(
        'setContext', 'keyboard-multicursor.hasMarks',
  ...
  const markCmd = vscode.commands.registerCommand(
      'keyboard-multicursor.markPosition', () => markCurrentPos(true));

  // Command: apply all marks as real cursors
  const applyCmd = vscode.commands.registerCommand(
      'keyboard-multicursor.applyCursors', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
  ...
  const markAndGotoNext = vscode.commands.registerCommand(
      'keyboard-multicursor.markAndGotoNext', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
  ...
  const markAndGotoPrev = vscode.commands.registerCommand(
      'keyboard-multicursor.markAndGotoPrev', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
  ...
  const clearCmd =
      vscode.commands.registerCommand('keyboard-multicursor.clearMarks', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
        clearAll(editor);
      });
  ...
  const onEditorChange = vscode.window.onDidChangeActiveTextEditor(editor => {
    ...
    updateStatusBar();
    vscode.commands.executeCommand(
        'setContext', 'keyboard-multicursor.hasMarks', false);
  });
